...
- **Multi-source search** - aggregates torrents from 12 indexers (piratebay, hiddenbay, 1337x, yts, nyaasi, limetorrent, torrentproject, pornrips, sukebei, knaben_adult, xxxclub, mypornclub) in parallel using goroutines
...
├── stremio_providers.go    Stremio studio/cover provider wiring
├── internal/
...
│   ├── stremio/            Stremio protocol core (manifest, catalog, meta, streams) + magnetio edge handler
│   └── services/
│       ├── hentai/         Hentai catalog/meta resolvers
│       ├── images/         Image extractor dispatcher and host extractors
│       ├── jobs/           Background job runner
│       ├── metadata/       TPDB/StashDB reference metadata lookups
...
│       └── streams/        Magnetio-compatible stream resolution
├── cmd/                    Standalone CLI tools (ingest, audit, prune, recount, ...)
├── pkg/
│   ├── models/             Shared model types
│   ├── mongo/              MongoDB storage driver
│   └── storage/            Storage interface
├── dashboard/              Next.js admin dashboard source
...
### Stremio Protocol
- `GET /stremio/:config/manifest.json` - addon manifest
- `GET /stremio/:config/catalog/:type/:catalogFile` - catalog feeds
- `GET /stremio/:config/meta/:type/:metaFile` - meta resolution
- `GET /stremio/:config/stream/:type/:streamFile` - stream resolution (JSON 404 on Go; resolved on the Node edge)

### Magnetio Protocol
- `GET /magnetio/manifest.json` - dummy manifest
- `GET /magnetio/:config/{manifest,catalog,meta,stream}` - per-config Stremio protocol for the Magnetio addon
- `GET /providers` - list Magnetio providers
- `GET /streams/:type/:id` - Magnetio-compatible stream resolution

### AtishMKV
- `GET /atishmkv/status` - catalog sync status
- `POST /atishmkv/sync` - run catalog sync
- `POST /atishmkv/refresh` - refresh direct links

### Addon Integration
- `GET /api/addon-status` - public addon status reports
- `POST /api/addon/meta-enqueue` - enqueue meta lookups (auth)

### Monitoring (IP + dashboard-password gated)
- `GET /api/monitoring/dashboard` - aggregate dashboard data
- `GET /api/monitoring/logs` - recent application logs
- `POST /api/monitoring/<job>-trigger` - trigger any background job
...
- [stream-frontend](https://github.com/akshatsinghkaushik/stream-frontend) - React 19 + TypeScript frontend (the `torrent-browse-ui` clone) that consumes this API
- [stremio-tpb-porn](https://github.com/akshatsinghkaushik/stremio-tpb-porn) - Node Stremio addon edge (the `tpb-stremio-addon` clone) that proxies this backend's manifest/catalog/meta
- [Magnetio-addon](https://github.com/akshatsinghkaushik/Magnetio-addon) - Stremio addon served via the `/magnetio/*` routes on this backend
