...
```json
{ "success": true, "websites": ["piratebay", "hiddenbay", "1337x", "yts", "nyaasi", "limetorrent", "torrentproject", "pornrips", "sukebei", "knaben_adult", "xxxclub", "mypornclub"] }
```
...
The Go backend serves the Stremio addon protocol for xxx catalogs (piratebay/HiddenBay only). Stream resolution is intentionally not served here - per-user debrid keys stay on the Node edge - but the route is registered so it returns a clean JSON 404 instead of falling through to static HTML.

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/stremio/:config/manifest.json` | Addon manifest (config-encoded) |
| `GET` | `/stremio/:config/catalog/:type/:catalogFile` | Catalog feed (optional `/:extraFile` skip/skipped) |
| `GET` | `/stremio/:config/meta/:type/:metaFile` | Meta resolution |
| `GET` | `/stremio/:config/stream/:type/:streamFile` | Stream resolution (JSON 404 on the Go edge) |
| `OPTIONS` | `/stremio/:config/*` | CORS preflight |

## Magnetio Protocol

The Node Magnetio addon can proxy manifest/catalog/meta here; stream resolution stays on the Node edge.

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/magnetio/manifest.json` | Dummy manifest |
| `GET` | `/magnetio/:config/manifest.json` | Per-config manifest |
| `GET` | `/magnetio/:config/catalog/:type/:catalogFile` | Catalog feed (optional `/:extraFile`) |
| `GET` | `/magnetio/:config/meta/:type/:metaFile` | Meta resolution |
| `GET` | `/magnetio/:config/stream/:type/:streamFile` | Stream resolution (delegated to the edge) |

### `GET /providers`

Lists the Magnetio-compatible providers. No auth.

### `GET /streams/:type/:id`

Magnetio-compatible stream resolution for a `type`/`id` pair. No auth.

## AtishMKV

Catalog sync and direct-link refresh for the AtishMKV source.
...
Public addon status reports. Optional `/:addonId` to fetch a single addon.

### `POST /api/addon/meta-enqueue`

Enqueue metadata lookups for the background meta enricher. Requires auth.

**Body**: `{ "items": [{ ...MetaEnqueueItem }] }` -> `{ "success": true, "added": <count> }`
