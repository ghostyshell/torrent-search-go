...
4. `scraper.NewService()` + `RegisterScraper()` x 13 - builds the scraper registry
...
| `x1337.go` | `X1337Scraper` | 1337x (via the 1337xx.to mirror, direct HTTP) |
...
| `pornrips.go` | `PornripsScraper` | PornRips.to release blog (detail-page magnet extraction) |
| `sukebei.go` | `SukebeiScraper` | Sukebei HTML |
| `knaben_adult.go` | `KnabenAdultScraper` | Knaben adult API endpoint |
| `xxxclub.go` | `XxxClubScraper` | XXXClub HTML |
| `mypornclub.go` | `MypornClubScraper` | MyPornClub HTML |
...
| `docker-compose.yml` | Local compose setup (Go backend only) |
