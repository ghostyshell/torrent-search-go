...
`internal/services/scraper.Service` is the central scraper registry. At startup, `main.go` registers 13 scrapers - each targeting a different public torrent indexer. Every scraper gets the shared `*http.Client` (30 s timeout, 100 max idle connections) to avoid redundant connection pools.
...
scraperService.RegisterScraper("sukebei",        sukebeiScraper)
scraperService.RegisterScraper("knaben_adult",   knabenAdultScraper)
scraperService.RegisterScraper("xxxclub",        xxxClubScraper)
scraperService.RegisterScraper("mypornclub",     mypornClubScraper)
```
