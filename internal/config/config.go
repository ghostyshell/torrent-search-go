...
type BackgroundJobsConfig struct {
	...
	ReferenceWarmer            JobScheduleConfig              `json:"referenceWarmer"`
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		Cache: CacheConfig{
			StreamUrlTTLSeconds:    getMaxInt(60, getEnvAsInt("STREAM_URL_TTL_SECONDS", 12*60*60)),
		...
		BackgroundJobs: BackgroundJobsConfig{
			...
			ReferenceWarmer: JobScheduleConfig{
				Interval:     time.Duration(getMaxInt64(60*60*1000, getEnvAsInt64("REF_WARMER_INTERVAL_MS", 60*60*1000))) * time.Millisecond,
				InitialDelay: time.Duration(getMaxInt64(0, getEnvAsInt64("REF_WARMER_INITIAL_MS", 90*1000))) * time.Millisecond,
			},
		...
		Scraper: ScraperConfig{
			FlareSolverrURL: os.Getenv("FLARE_SOLVERR_URL"),
		},
		S3: S3Config{
