...
func (h *AuthHandler) GoogleLogin(w http.ResponseWriter, r *http.Request) {
	...
	// select_account lets users pick/switch their Google account without
	// forcing a full consent re-authorization (and the "unverified app"
	// warning) on every login. consent was forcing the trust prompt each
	// re-login, which recurs whenever a refresh logs the user out.
...
func (h *AuthHandler) ExchangeAuthCode(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Transient store error during the exchange round-trip; don't 401 and
		// force a fresh OAuth login - let the client retry.
...
func (h *AuthHandler) ValidateSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Transient store error (e.g. the Mongo pool not yet ready right after a
		// restart/redeploy). Do NOT conflate this with an invalid session: a 401
		// here makes the frontend wipe a still-valid 30-day token and log the
		// user out on every refresh that lands during the brief window. Surface
		// 503 so the client treats it as transient and retries/keeps the token.
		writeJSON(w, http.StatusServiceUnavailable, map[string]interface{}{
			"success": false,
			"error":   "Session store temporarily unavailable",
			"code":    "STORE_UNAVAILABLE",
