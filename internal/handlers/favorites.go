...
func (h *FavoritesHandler) AddFavorite(w http.ResponseWriter, r *http.Request) {
	...
	// Persist the cover durably into the images collection (keep/ + storage_key) so
	// it gets re-signed on read and is never temp-cleaned. The add-time URL stored
	// on favorite_entries is a presigned temp URL that expires in ~7 days; without
	// this, any favorite older than a week renders a blank cover. Fire-and-forget
	// so AddFavorite latency is unaffected; skip only if a row already has a
	// storage_key (a row with a raw external URL and no storage_key still needs
	// upgrading, so re-attempt it).
	if req.CoverImageURL != "" {
		go func(key, coverURL string) {
			defer func() {
				if rec := recover(); rec != nil {
					log.Printf("[AddFavorite] durable cover persist panicked for %s: %v", key, rec)
				}
			}()
			bgCtx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
			defer cancel()
			// Skip only when a permanent (non-temp) storage key already exists.
			if existing, _ := h.storage.GetCoverImageByKey(bgCtx, key); existing != nil && existing.StorageKey != nil && *existing.StorageKey != "" && !strings.Contains(*existing.StorageKey, "/temp/") {
				return
			}
			if err := h.storage.SetCoverImage(bgCtx, key, coverURL, false); err != nil {
				log.Printf("[AddFavorite] durable cover persist failed for %s: %v", key, err)
			}
		}(torrentKey, req.CoverImageURL)
	}
...
func (h *FavoritesHandler) GetFavorites(w http.ResponseWriter, r *http.Request) {
	...
	// Build torrentKey → metaID (lowercase infoHash) from the magnets so we can
	// query shared_meta even when meta_id is not persisted on the images row.
	keyToMetaID := make(map[string]string, len(favorites))
	for _, fav := range favorites {
		if ih := magnetInfoHash(fav.MagnetLink); ih != "" {
			keyToMetaID[fav.TorrentKey] = strings.ToLower(ih)
		}
	}

	// Re-sign expired S3 URLs, then prefer TPDB/StashDB covers from shared_meta
	// (always a valid external URL) over stored temp S3 URLs that may have been
	// cleaned up. Promote temp/deleted covers to keep/ in the background.
	for key, imgRow := range coverImages {
		if imgRow == nil {
			continue
		}
		h.storage.refreshCoverPresignedURL(ctx, imgRow)
		metaID := keyToMetaID[key]
		resolved := resolveCover(ctx, h.storage, coverResolveInput{row: imgRow, metaID: metaID})
		if resolved.url != "" && resolved.url != imgRow.PixhostURL {
			// TPDB/StashDB cover from shared_meta — use it now, store to keep/ async.
			imgRow.PixhostURL = resolved.url
			go func(k, u string) {
				bgCtx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
				defer cancel()
				_ = h.storage.SetCoverImage(bgCtx, k, u, false)
			}(key, resolved.url)
		}
		coverImages[key] = imgRow
	}

	// Favorites with no images row (old favorites whose only cover is an expired
	// temp URL in torrent_data) never enter the loop above, so the recovered
	// shared_meta poster never reaches the UI. Resolve shared_meta for those
	// here and promote the poster to a durable keep/ row so future loads find it.
	metaCovers := make(map[string]string, len(favorites))
	for _, fav := range favorites {
		metaID := keyToMetaID[fav.TorrentKey]
		if metaID == "" {
			continue
		}
		if row := coverImages[fav.TorrentKey]; row != nil {
			continue
		}
		merged := loadMergedCoverMeta(ctx, h.storage, metaID)
		if merged.poster == "" {
			continue
		}
		metaCovers[fav.TorrentKey] = merged.poster
		go func(k, u string) {
			bgCtx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
			defer cancel()
			_ = h.storage.SetCoverImage(bgCtx, k, u, false)
		}(fav.TorrentKey, merged.poster)
	}

	// For favorites whose cover is still a temp S3 URL (object likely deleted after
	// 7 days), enqueue async TPDB/StashDB lookup so the next load finds a permanent cover.
	if h.MetaEnqueuer != nil {
		for _, fav := range favorites {
			imgRow := coverImages[fav.TorrentKey]
			isTempOrMissing := imgRow == nil ||
				(imgRow.StorageKey != nil && strings.Contains(*imgRow.StorageKey, "/temp/")) ||
				(imgRow.StorageKey == nil && strings.Contains(imgRow.PixhostURL, "/temp/"))
			if !isTempOrMissing {
				continue
			}
			// Already got TPDB from shared_meta — no need to re-enqueue.
			if imgRow != nil && imgRow.TpdbURL != nil && *imgRow.TpdbURL != "" {
				continue
			}
			ih := keyToMetaID[fav.TorrentKey]
			if ih == "" {
				continue
			}
			detailURL := ""
			if fav.TorrentData != "" {
				...
				if json.Unmarshal([]byte(fav.TorrentData), &td) == nil {
					detailURL, _ = td["Url"].(string)
				}
			}
			h.MetaEnqueuer(fav.TorrentName, detailURL, fav.Website, ih)
		}
	}
	...
	for _, fav := range favorites {
		item := mergeFavoriteItem(fav, coverImages, metaCovers)
...
func mergeFavoriteItem(fav *models.FavoriteRow, coverImages map[string]*models.ImageRow, metaCovers map[string]string) map[string]interface{} {
	...
	if imgRow := coverImages[fav.TorrentKey]; imgRow != nil {
		...
		// Primary URL: manual > tpdb > details > pixhost
		if imgRow.PixhostURL != "" {
			ci["url"] = imgRow.PixhostURL
		}
		if imgRow.TpdbURL != nil && *imgRow.TpdbURL != "" {
			ci["tpdbUrl"] = *imgRow.TpdbURL
			if ci["url"] == nil {
				ci["url"] = *imgRow.TpdbURL
			}
		}
		if imgRow.DetailsURL != nil && *imgRow.DetailsURL != "" {
			ci["detailsUrl"] = *imgRow.DetailsURL
			if ci["url"] == nil {
				ci["url"] = *imgRow.DetailsURL
			}
		}
		item["coverImage"] = ci
	}

	// Favorites with no images row fall through to the stale temp cover copied
	// from torrent_data above. When shared_meta has a recovered TPDB/StashDB
	// poster, prefer it so the UI shows a live cover instead of an expired URL.
	if poster := metaCovers[fav.TorrentKey]; poster != "" {
		if imgRow := coverImages[fav.TorrentKey]; imgRow == nil {
