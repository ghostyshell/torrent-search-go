...
func NewMonitoringHandler(storage *StorageProvider, cfg *config.Config) *MonitoringHandler {
	...
	return &MonitoringHandler{
		...
		taskStats: &backgroundTaskStats{
			...
			referenceWarmer:           &taskStats{intervalMs: 60 * 60 * 1000, status: "idle", results: make([]map[string]interface{}, 0)},
...
func (h *MonitoringHandler) InitScheduledNextRuns() {
	...
	setNext(h.taskStats.referenceWarmer, cfg.ReferenceWarmer.InitialDelay)
...
func (h *MonitoringHandler) updateTaskStats(taskName string, result map[string]interface{}, manual bool) {
	...
	case "referenceWarmer":
		t = h.taskStats.referenceWarmer
...
func (h *MonitoringHandler) serializeTaskStats() []map[string]interface{} {
	...
	return []map[string]interface{}{
		...
		taskStatMap("referenceWarmer", ts.referenceWarmer),
...
func (h *MonitoringHandler) GetBackgroundTaskStats(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]interface{}{
		...
		"tasks": map[string]interface{}{
			...
			"referenceWarmer":         h.taskStats.referenceWarmer,
		},
		"timestamp": time.Now().UTC().Format(time.RFC3339),
	})
}
...
type JobScheduler struct {
	...
	runner     interface {
		...
		ReferenceWarmer(ctx context.Context) (map[string]interface{}, error)
...
func (s *JobScheduler) startReferenceWarmer() {
	if !s.config.Redis.Enabled {
		return
	}
	cfg := s.config.BackgroundJobs.ReferenceWarmer
	s.runPeriodic("referenceWarmer", cfg.Interval, cfg.InitialDelay, func() {
		s.runReferenceWarmer(false)
	})
}

func (s *JobScheduler) runReferenceWarmer(manual bool) {
	if !s.config.Redis.Enabled {
		return
	}
	if s.monitoring != nil {
		s.monitoring.mu.Lock()
		now := time.Now()
		s.monitoring.taskStats.referenceWarmer.status = "running"
		s.monitoring.taskStats.referenceWarmer.lastRun = &now
		s.monitoring.mu.Unlock()
	}
	...
	result, err := RunWithJobFileLogging(s.config, "referenceWarmer", func(logLine func(string, string)) (map[string]interface{}, error) {
		logLine("info", "[ReferenceWarmer] Job started")
		results, runErr := s.runner.ReferenceWarmer(ctx)
		if runErr != nil {
			logLine("error", "[ReferenceWarmer] "+runErr.Error())
			return map[string]interface{}{"success": false, "error": runErr.Error(), "manual": manual}, runErr
		}
		for k, v := range results {
			logLine("info", fmt.Sprintf("[ReferenceWarmer] %s: %v", k, v))
		}
		return results, nil
	})
	...
	if s.monitoring != nil {
		...
		s.monitoring.updateTaskStats("referenceWarmer", result, manual)
	}
}
...
// runPeriodic runs fn once after initialDelay, then on every interval (mirrors Node setTimeout + setInterval).
