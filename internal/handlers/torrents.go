...
func (h *TorrentHandler) enrichCoverImages(ctx context.Context, fallbackWebsite string, results []models.Torrent) {
	...
	for i := range results {
		...
		go func(t *models.Torrent, website, key, metaID string) {
			...
			if resolved.url != "" {
				t.CoverImage = &models.CoverImage{Type: "url", URL: resolved.url}
			...
			t.CoverImage = &models.CoverImage{Type: "url", URL: details.CoverImageURL}

			if h.storage != nil && key != "" {
				_ = h.storage.SetCoverImage(ctx, key, details.CoverImageURL, true)
			}
