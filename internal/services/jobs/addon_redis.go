...
var (
	...
	prefixStashTagCache = cache.PrefixStashTag
)
...
func (s *addonRedisStore) ExistsManyShared(ctx context.Context, source string, keys []string) ([]bool, error) {
	...
	// Poster-aware: a shared_meta entry with an empty poster (written by a
	// pre-clean-query wrong/no-poster match) must not short-circuit re-lookup,
	// or TPDB-matchable favorites never get re-probed with the now-correct
	// query and cover-image keeps returning "not found". Treat empty-poster
	// entries as absent so the MetaEnricher re-probes and overwrites them.
	...
	for i, v := range raw {
		...
		b, _ := json.Marshal(v)
		var m SharedMeta
		if json.Unmarshal(b, &m) == nil && strings.TrimSpace(m.Poster) != "" {
			out[i] = true
		}
	}
	return out, nil
