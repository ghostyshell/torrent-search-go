...
const (
	...
	defaultMetaMaxPerTick  = 150
	defaultMetaMaxQueue    = 5000
...
func (r *Runner) EnqueueMetaLookups(items []MetaEnqueueItem) int {
	...
	for _, it := range items {
		...
		if !ok {
			...
			r.metaQueue.items[id] = pendingMeta{id: id, title: title, detailURL: detailURL, website: website}
			added++
			continue
		}
...
func (r *Runner) MetaEnricher(ctx context.Context) (map[string]interface{}, error) {
	...
	runTPDB := func(item pendingMeta) {
		...
		sem <- struct{}{}
		defer func() { <-sem }()
		meta, err := tpdb.SearchMetadata(ctx, item.title)
		if err != nil {
			if errors.Is(err, metadata.ErrTPDBRateLimited) {
	...
	runStash := func(item pendingMeta) {
		...
		sem <- struct{}{}
		defer func() { <-sem }()
		meta, err := stash.SearchMetadata(ctx, item.title, item.detailURL)
		if err != nil {
			failed.Add(1)
			return
		}
