...
func (r *Runner) PornripsEnrich(ctx context.Context) (map[string]interface{}, error) {
	...
	// First pass: never-attempted entries (newest-first). This is the "full schbang"
	// for newly-ingested posts. tickStart is captured before the first pass so the
	// re-enrich pass below can exclude entries this tick just enriched: a same-tick
	// miss has not had time for TPDB/Stash to add the scene, so retrying it
	// immediately is pure waste. Only previous-tick misses are worth re-trying.
	tickStart := time.Now().Unix()
	entries, err := r.storage.GetPornripsEntriesMissingEnrichment(ctx, perTick)
	if err != nil {
		return nil, err
	}
	attempted, enriched := r.enrichPornripsBatch(ctx, entries, tpdbPool, stashPool, redisStore, useTPDB, useStash, concurrency)

	// Re-enrich pass: recently-ingested entries the sweep already attempted (in a
	// prior tick) but that produced no scene match (resolved_title empty). TPDB/Stash
	// typically add a scene days after the WP post, so a once-missed entry gets a
	// second chance while it is still recent. Bounded by a 14-day window (entries
	// age out so true-misses stop being retried, capping TPDB budget) and a small
	// per-tick budget. enrichOne is idempotent: writes shared_meta if TPDB/Stash now
	// has the scene, no-op on a continued miss.
	const reEnrichWindow = 14 * 24 * time.Hour
	reEntries, rerr := r.storage.GetPornripsEntriesForReEnrichment(ctx,
		time.Now().UTC().Add(-reEnrichWindow).Format("2006-01-02"), tickStart, perTick/2)
	var reAttempted, reEnriched int
	if rerr == nil {
		reAttempted, reEnriched = r.enrichPornripsBatch(ctx, reEntries, tpdbPool, stashPool, redisStore, useTPDB, useStash, concurrency)
	}

	out := map[string]interface{}{
		"success":       true,
		"scanned":       len(entries) + len(reEntries),
		"attempted":     attempted + reAttempted,
		"enriched":      enriched + reEnriched,
		"reScanned":     len(reEntries),
...
// enrichPornripsBatch runs the enrich worker pool over one batch of entries and
// returns (attempted, enriched). Shards entries round-robin across the client
// pool so each key's 250ms throttle lock runs in parallel. nil client when that
// source is disabled (enrichOne checks the use flags, not the pointer).
func (r *Runner) enrichPornripsBatch(ctx context.Context, entries []models.PornripsEntry, tpdbPool []*metadata.TPDBClient, stashPool []*metadata.StashDBClient, store *addonRedisStore, useTPDB, useStash bool, concurrency int) (int, int) {
	if len(entries) == 0 {
		return 0, 0
	}
	var (
	...
	for i, e := range entries {
		...
		go func(entry models.PornripsEntry, idx int) {
			...
			ok := r.enrichOnePornripsEntry(ctx, entry, tpdb, stash, store, useTPDB, useStash)
			mu.Lock()
			attempted++
			if ok {
				enriched++
