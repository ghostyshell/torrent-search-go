...
const (
	...
	// maxCatalogSkip caps the user-supplied ?skip= before it reaches a store or
	// scrape read. Without it a huge skip forces Mongo to walk+discard deep into
	// the {date:-1} index (enriched_scenes/pornrips_entries) or the compact path
	// to fan out 20 variant pages, a cheap DoS. Stremio browsing never paginates
	// this deep.
	maxCatalogSkip = 10000

	// searchScrapeTimeout caps the cold global `search` catalog scrape
	// (StashDB performer resolve + piratebay fanout over up to 16 terms). A slow
	// StashDB (8s) + slow piratebay.org (up to 30s/term) can reach 15-20s, exceeding
	// Stremio's catalog request timeout - Stremio then retries before the cache
	// populates, looping into multi-minute "no results". The bound cancels
	// in-flight requests at 9s, returns whatever completed, and the caller caches
	// it so the retry hits the cache. StashDB resolve (searchResolveTimeout, 3s)
	// is a sub-deadline of this. ponytail: a single global cap; per-query adaptive
	// timeouts if the 9s budget proves wrong for a source set.
	searchScrapeTimeout = 9 * time.Second
)
...
func (h *Handler) ServeCatalog(ctx context.Context, cfg Config, contentType, catalogID, extra string) (CatalogResponse, error) {
	...
	if strings.HasPrefix(catalogID, "tpdb_") {
		return h.serveTPDBCatalog(ctx, cfg, contentType, catalogID, searchQ, skip)
	}

	if strings.HasPrefix(catalogID, "hentai_") {
		return h.serveHentaiCatalog(ctx, catalogID, genre, searchQ, skip)
	}
	...
	// Any registered tube source (pvz_ / fpv_ / and later ypv_ / wpt_ / pec_ /
	// phd_ / p4d_ / hqp_) routes through the generic handler. h.TubeSources is
	// nil only in bare-handler tests; a cold install still registers sources
	// (with nil resolvers), so the catalog path works without a runner.
	if h.TubeSources != nil {
		if src := h.TubeSources.LookupByCatalogPrefix(catalogID); src != nil {
			return h.serveTubeCatalog(ctx, src, catalogID, genre, searchQ, skip)
		}
	}
	...
	// The global `search` catalog is never pre-warmed, so every first query is a
	// cold piratebay fanout. Skip request-path live meta (TPDB/Stash per-torrent
	// resolve, 22s timeout) for search: it runs after the 9s scrape bound and would
	// push the total past Stremio's catalog timeout, re-triggering the retry loop
	// (the multi-minute symptom). Posters come from the torrent record (none for
	// piratebay) and the async MetaEnqueuer populates shared_meta, so the next
	// search within the shared_meta TTL serves posters from cache.
	skipLiveMeta := isPornSearchCatalogID(catalogID)
	metas, err := h.buildMetas(ctx, cfg, torrents, contentType, quality, skipLiveMeta)
	if err != nil {
		return CatalogResponse{Metas: []MetaPreview{}}, err
	}
...
func isCatalogAllowed(cfg Config, catalogID string) bool {
	...
	// Tube sources: any registered source's catalog prefix is "own" (not a TPB
	// sort catalog), and disabling the source's Key() disables all its catalogs.
	tubeDisableKey := ""
	if defaultTubeRegistry != nil {
		if src := defaultTubeRegistry.LookupByCatalogPrefix(catalogID); src != nil {
			isOwn = true
			tubeDisableKey = src.Key()
		}
	}
	base := catalogID
	if !isOwn {
	...
	if isOwn {
		for _, d := range cfg.DisabledCatalogs {
			if d == base ||
				(strings.HasPrefix(catalogID, "sukebei_") && d == "sukebei") ||
				(tubeDisableKey != "" && d == tubeDisableKey) {
				return false
			}
		}
		return true
...
func (h *Handler) serveCompactMergedCatalog(ctx context.Context, cfg Config, base4k, baseFhd string, only1080 bool, contentType string, skip, maxResults int) (CatalogResponse, error) {
	...
	metas, err := h.buildMetas(ctx, cfg, pageTorrents, contentType, "", false)
	if err != nil {
		return CatalogResponse{Metas: []MetaPreview{}}, err
	}
...
// serveCategoryCatalog is the store-backed tpdb_cat / stashdb_cat category
// catalog. "All" is one store query (source + matched_sources gate, no tag); a
// specific genre filters by tags_norm $in over enrichedTagTokens(genre), which
// expands NormToken(category.StashTag) with the compound tokens TPDB/StashDB
// actually tag scenes with (milf30, analsex, blondhair, footfetish, ...). This
// generalizes the CategoryWarmer, which only scraped TPB - the store covers
// every source the EnrichedScenesSync job matched. The {source, matched_sources
// $in} gate is the source-config fix: a scene surfaces only when one of the
// user's configured torrent sources resolved it. Cold store (EnrichedScenes
// nil) -> empty. The CategoryWarmer still runs and writes its (now unread)
// Redis blobs as a fallback until it is retired.
func (h *Handler) serveCategoryCatalog(ctx context.Context, catalogID, genre string, skip, maxResults int, cfg Config) (CatalogResponse, error) {
	...
	gates := matchedSourcesFilter(cfg)
	if len(gates) == 0 {
		return CatalogResponse{Metas: []MetaPreview{}}, nil
	}
	var tags []string
	if genre != "All" {
		tags = enrichedTagTokens(genre)
		if len(tags) == 0 {
			return CatalogResponse{Metas: []MetaPreview{}}, nil
		}
	}
	...
	scenes, err := h.EnrichedScenes.GetEnrichedScenesByMatchedSources(ctx, source, tags, gates, skip, maxResults)
	if err != nil {
		return CatalogResponse{Metas: []MetaPreview{}}, nil
	}
	return CatalogResponse{Metas: enrichedScenesToMetas(scenes)}, nil
}

// categoryTagNorm returns NormToken of the StashTag for the category named name,
// or "" when no category matches. The StashTag is the canonical tag string used
// for discovery, so its normalized form is what tags_norm stores.
func categoryTagNorm(name string) string {
	for _, c := range jobs.AllCategories {
		if c.Name == name {
			return prmodels.NormToken(c.StashTag)
		}
	}
	return ""
}

// enrichedTagAliases maps a tpdb_cat/stashdb_cat genre (NormToken(StashTag)) to
// the extra tags_norm tokens enriched_scenes holds for that concept beyond the
// bare token. The store-backed category catalog queries tags_norm $in over
// [bare] + these (enrichedTagTokens), because TPDB/StashDB tag scenes with
// compound forms the bare category name does not collapse to:
//
//	"MILF" -> milf30 (TPDB "MILF (30+)"), "Anal" -> analsex (StashDB "Anal Sex"),
//	"Blonde" -> blondhair, "Feet" -> footfetish, "Tattoo" -> tattoos, ...
//
// Curated from the enriched_scenes vocabulary audit 2026-06-30 (cmd/tpbverify),
// NOT the pornrips_entries prTagAliases - the two stores' tag vocabularies
// differ (e.g. enriched has footfetish=3381; pornrips has cumonfeet). Positive-
// concept tokens only (no "notattoos"/"prematureejaculation" false positives).
// The source filter partitions TPDB vs StashDB, so a token present on only one
// source is a no-op $in member for the other. Categories not listed fall
// through to the bare NormToken (the original exact-match behaviour). Refresh
// if the TPDB/StashDB genre taxonomy shifts.
var enrichedTagAliases = map[string][]string{
	"milf":    {"milf30", "milfmilffantasy", "xxxmilfvideos", "milfsin4k", "blondemilf", "latinamilf", "nudemilf", "asianmilf", "bustymilf", "milfblowjob", "milfkissing", "curvymilf"},
	"teen":    {"teen1822", "teengirl1822", "teenpussy"},
	"mature":  {"mature30", "nudemature", "maturepussy", "maturemasturbating", "matureslut"},
	"latina":  {"latinawoman", "latinatrans", "latinamilf", "latinasuckingcock", "latinafeet"},
	"ebony":   {"ebonyass", "ebonyblackmailfantasy", "blackwoman", "ethnicityblack"},
	"blonde":  {"blondhair", "blondehairfemale", "blondegoddess", "blondemilf"},
	"redhead": {"redhairfemale", "redhair"},
	// "roughsex": both stores tag with bare "rough", not "roughsex" (TPDB rough=2786,
	// StashDB rough=23; roughsex=1). Without this alias the Rough Sex genre matched 1/0.
	"roughsex": {"rough"},
	// "pov": TPDB has bare pov=5086 (matched), but StashDB has NO bare pov - it tags
	// position-pov compounds (malepov, cowgirlpov, ...). Without these the StashDB POV
	// genre matched 0. nonpov/mixedpov are excluded (negative/ambiguous).
	"pov":               {"malepov", "blowjobpov", "cowgirlpov", "doggystylepov", "reversecowgirlpov", "missionarypov", "analcowgirlpov", "analdoggystylepov", "analmissionarypov", "handjobpov", "kissingpov", "titjobpov", "rimmingpov", "tinymanpov", "povpegging", "povfacial", "doggypov"},
	"petite":            {"bodypetite", "petitegoddesspetiteprincess", "petiteblonde", "petitelatina", "petitegirl", "petitemilf", "petiteskinny"},
	"tattoo":            {"tattoos", "tattooedbabes", "tattoospiercings", "tattooedwoman", "armtattoo", "backtattoo", "legtattoo", "girlswithtattoos", "heavilytattooed", "hiptattoo", "necktattoo", "handtattoo", "facetattoo", "tattooedman", "fingertattoo", "sidetattoo", "shouldertattoo", "chesttattoo", "pussytattoo", "stomachtattoo", "tattooedbigass"},
	"doublepenetration": {"doublepenetrationdp", "doubleanalpenetrationdap", "doublevaginalpenetrationdvp"},
	"public":            {"publicnudity", "publicoutdoors", "publicsex", "publicagent"},
	"stepfamily":        {"stepmother", "stepsister", "stepdad", "stepdaughter", "stepsiblings", "stepson", "stepbrother", "stepaunt", "stepuncle"},
	"cuckold":           {"cuckolding", "cuckoldhumiliation", "cuckoldhusband", "cuckqueaning", "cuckoldress", "cuckoldhandholding", "cuckoldpov"},
	"pissing":           {"pisslover", "pisskink", "pissingfetish", "pissreceptacle", "severalstreamsofpissinmouth", "pissinmouth", "desperatepissing", "pissdrinking", "pissplay", "desperatetopiss", "pissinass"},
	"feet":              {"footfetish", "footjob", "foottorment", "barefoot", "footworship", "footsex", "foottease", "cumonfeet", "hdfeetporn", "feetworship", "closeupfeet", "malefeet", "bigfeet", "feettease", "oilyfeet", "oiledfeet", "footpov", "footsmelling"},
	"anal":              {"analsex"},
}

// enrichedTagTokens resolves a tpdb_cat/stashdb_cat genre to the tags_norm $in
// tokens to query: the bare NormToken(StashTag) plus any compound aliases from
// enrichedTagAliases. Returns nil for an unknown genre (no matching category).
func enrichedTagTokens(genre string) []string {
	n := categoryTagNorm(genre)
	if n == "" {
		return nil
	}
	toks := []string{n}
	if a, ok := enrichedTagAliases[n]; ok {
		toks = append(toks, a...)
	}
	return toks
}

// EnrichedTagTokens is the exported form of enrichedTagTokens for read-only
// tooling (cmd/tpbaudit) so it can replicate the deployed tpdb_cat/stashdb_cat
// query without duplicating the alias map.
func EnrichedTagTokens(genre string) []string { return enrichedTagTokens(genre) }
...
func (h *Handler) fetchAdultCatalog(ctx context.Context, cfg Config, catalogID, searchQ string, skip int) ([]catalogTorrent, error) {
	...
	switch {
	case searchQ != "":
		// Bound the cold search scrape so a slow StashDB resolve + piratebay
		// fanout (up to 16 terms at 30s/term) cannot exceed Stremio's catalog
		// request timeout. On deadline, in-flight requests are canceled and
		// whatever completed so far is returned + cached, so Stremio's retry
		// hits the cache instead of re-scraping into a multi-minute loop.
		scrapeCtx, cancel := context.WithTimeout(ctx, searchScrapeTimeout)
		terms := h.resolveSearchTerms(scrapeCtx, cfg, searchQ)
		raw, err = h.searchPirateBayTerms(scrapeCtx, terms, page, models.SearchOptions{Sort: "7"})
		raw = append(raw, h.fanoutAdultSources(scrapeCtx, cfg, searchQ, page, "7")...)
		cancel()
	case params.Query != "":
...
func (h *Handler) fanoutAdultSources(ctx context.Context, cfg Config, query string, page int, sort string) []models.Torrent {
	...
	if cfg.ExtraIndexers {
		sources = append(sources, "knaben_adult", "mypornclub")
	}
	if cfg.Enable1337x {
...
// poisons the shared cache for installs that left Knaben/xxxclub off.
func catalogFanoutKey(cfg Config) string {
	parts := []string{"hb"}
	if cfg.ExtraIndexers {
		parts = append(parts, "kn", "xc", "mp")
	}
	if cfg.Enable1337x {
...
func filterByIndexerConfig(torrents []catalogTorrent, cfg Config) []catalogTorrent {
	...
	for _, t := range torrents {
		...
		switch src {
		case "knaben", "xxxclub", "mypornclub":
			if !cfg.ExtraIndexers {
				continue
			}
		case "1337x":
...
func (h *Handler) fanoutAdultBrowse(ctx context.Context, cfg Config, category string, page int) []models.Torrent {
	...
	sources := []string{"xxxclub", "mypornclub"}
	type result struct {
		torrents []models.Torrent
	}
	ch := make(chan result, len(sources))
...
func (h *Handler) buildMetas(ctx context.Context, cfg Config, torrents []catalogTorrent, contentType, quality string, skipLiveMeta bool) ([]MetaPreview, error) {
	if len(torrents) == 0 {
	...
	for _, t := range torrents {
		...
		id := EncodeItemID(rec)
		// A scene group (multi-resolution rips of one PornRips scene) carries its
		// variants in Members; emit a jstrg: group id so the stream route returns one
		// stream per variant. The representative record still feeds setTorrent + the
		// meta preview below; the jstrg: meta branch re-encodes member[0] for lookup.
		if len(t.Members) > 0 {
			id = EncodeGroupID(t.Members)
		}
		rec.ID = id
		if store != nil {
		...
		// PornRips WP titles are dotted release filenames; render a readable
		// name from the Mongo title when no enriched shared_meta supplied one.
		// No live WP/reference lookup on the request path.
		if rec.Website == "pornrips" && (merged == nil || merged.Title == "") {
			name = strings.TrimSpace(strings.ReplaceAll(rec.Title, ".", " "))
		}
		...
		// The background PornripsSync job is the sole PornRips metadata populator;
		// never enqueue pornrips rows for the on-demand MetaEnricher (which would
		// live-probe TPDB/Stash). Other sources enqueue unchanged.
		if rec.Website != "pornrips" {
...
// shouldLiveResolve reports whether a catalog entry with no shared_meta should be
// resolved live against TPDB/Stash. PornRips is Mongo-only: the background
// PornripsSync job is the sole metadata populator, so pornrips rows never live-
// resolve. Other sources skip once they have a real cover to avoid redundant
// TPDB calls.
func shouldLiveResolve(website, poster string) bool {
	if website == "pornrips" {
		return false
	}
...
func (h *Handler) enrichCatalogLiveMeta(ctx context.Context, cfg Config, torrents []catalogTorrent, metas []MetaPreview, noMerged []bool) {
	...
	for i, t := range torrents {
		...
		// PornRips is Mongo-only: the background PornripsSync job is the sole
		// metadata populator, so shouldLiveResolve returns false for pornrips and
		// those rows are skipped here. Other sources skip once they have a real
		// cover to avoid redundant TPDB calls.
		if !shouldLiveResolve(t.Website, metas[i].Poster) {
			continue
		}
		if t.Title == "" {
			continue
