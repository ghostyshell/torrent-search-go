...
func TestCatalogFanoutKey(t *testing.T) {
	...
	if got := catalogFanoutKey(Config{ExtraIndexers: true}); got != "hb,kn,xc,mp" {
		t.Fatalf("extra indexers = %q", got)
	}
	if got := catalogFanoutKey(Config{ExtraIndexers: true, Enable1337x: true}); got != "hb,kn,xc,mp,1337" {
