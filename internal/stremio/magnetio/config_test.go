...
func TestParseConfigProviders(t *testing.T) {
	cfg := ParseConfig("providers=nyaa,bitsearch|limit=25|rd=abc123")
	if len(cfg.Providers) < 3 {
		t.Fatalf("expected at least 3 providers (nyaa + bitsearch + knaben core), got %v", cfg.Providers)
	}
	if cfg.Limit != 25 {
...
func TestParseConfigAliases(t *testing.T) {
	cfg := ParseConfig("providers=s8,s13")
	...
	if !seen["bitsearch"] {
		t.Fatal("expected s13 alias to resolve to bitsearch")
	}
}
