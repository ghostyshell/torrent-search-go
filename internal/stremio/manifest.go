...
var pornripsCatalogDefs = []prCatalogDef{
	...
	{id: "pr_tag", name: "PornRips · Tag", genre: true, options: pornripsTags, hideFromHome: true},
...
func BuildManifest(cfg Config, baseURL string, env *appconfig.Config, extraStudios []string) map[string]interface{} {
	...
	if containsSource(sources, "pornrips") {
		pornripsCatalogs = pornripsManifestCatalogs(disabled, homeGenre, cfg.DisabledPrStudios)
	}
	...
	catalogs := make([]map[string]interface{}, 0,
		len(tpbCatalogs)+len(pornripsCatalogs)+len(hentaiCatalogs)+len(sukebeiCatalogs)+len(stripchatCatalogs)+len(categoryCatalogs))
	catalogs = append(catalogs, tpbCatalogs...)
	catalogs = append(catalogs, pornripsCatalogs...)
	catalogs = append(catalogs, hentaiCatalogs...)
	catalogs = append(catalogs, sukebeiCatalogs...)
	catalogs = append(catalogs, stripchatCatalogs...)
	catalogs = append(catalogs, categoryCatalogs...)
...
func pornripsManifestCatalogs(disabled map[string]struct{}, homeGenre []map[string]interface{}, disabledPrStudios []string) []map[string]interface{} {
