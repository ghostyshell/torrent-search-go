...
const (
	...
	manifestDescription = "4K, 1080p & VR adult torrent catalogs from HiddenBay, PornRips, and Sukebei, plus HentaiMama episodes and Stripchat live HLS cams (240p-1080p). JAV catalog and ThePornDB scene browser with optional TPDB/StashDB metadata. Streams via 12 debrid providers (Real-Debrid, TorBox, and more) with P2P fallback and optional MediaFlow proxy. Works in Stremio and Nuvio; toggle sources, compact studio catalogs, and save encrypted cross-device presets."
)
...
// tubeGenreSet is the precomputed top-N studio/tag/performer option lists for
// one tube source's discover catalogs. The background sync job precomputes
// these from store distinct-aggregations and writes them to KV (the source's
// CachePrefixes().Genres prefix); the manifest path reads the KV blob once and
// passes it here so it never runs an aggregation on the per-request manifest
// path (which drove Mongo CPU to 100% - see the pr_tag note above). Cold install
// = empty set = Studio/Tag/Performer catalogs stay hidden until the first sync
// tick populates KV.
type tubeGenreSet struct {
	Studios    []string
	Tags       []string
	Performers []string
}

// tubeGenreOptions is the per-source genre map keyed by TubeSource.Key(). One
// entry per registered source.
type tubeGenreOptions = map[string]tubeGenreSet

// tubeCatalogDefs declares the five catalogs for a registered tube source.
// Recent is a visible homepage row; Studio/Tag/Performer are genre-pickers
// hidden from Home (only emitted when the sync job has precomputed their
// options); Search is a search-only row. type is Porn. The id prefix and
// display name come from the source, so adding a source needs no manifest edit.
func tubeCatalogDefs(src TubeSource, opts tubeGenreSet) []externalCatalogDef {
	p := src.CatalogPrefix()
	dn := src.DisplayName()
	defs := []externalCatalogDef{
		{id: p + "_recent", name: dn + " · Recent"},
		{id: p + "_search", name: dn + " · Search", search: true, hideFromHome: true},
	}
	if len(opts.Studios) > 0 {
		defs = append(defs, externalCatalogDef{id: p + "_studio", name: dn + " · Studio", options: opts.Studios, hideFromHome: true})
	}
	if len(opts.Tags) > 0 {
		defs = append(defs, externalCatalogDef{id: p + "_tag", name: dn + " · Tag", options: opts.Tags, hideFromHome: true})
	}
	if len(opts.Performers) > 0 {
		defs = append(defs, externalCatalogDef{id: p + "_performer", name: dn + " · Performer", options: opts.Performers, hideFromHome: true})
	}
	return defs
}

// tubeIDPrefixes returns the IDPrefix of every registered tube source in
// registration order, for the manifest idPrefixes list.
func tubeIDPrefixes(sources []TubeSource) []string {
	out := make([]string, 0, len(sources))
	for _, s := range sources {
		out = append(out, s.IDPrefix())
	}
	return out
}
...
// tubeOpts carries the precomputed tube-source discover options (from KV; empty
// on a cold install so Studio/Tag/Performer catalogs stay hidden until first sync).
func BuildManifest(cfg Config, baseURL string, env *appconfig.Config, extraStudios []string, prTagOpts []string, tubeOpts tubeGenreOptions, tubeSources []TubeSource) map[string]interface{} {
	...
	// Tube sources (pvz / fpv / and later ypv / wpt / pec / phd / p4d / hqp):
	// one loop over the registered sources, emitting each enabled source's five
	// catalogs. Adding a source = register it; no manifest edit.
	var tubeCatalogs []map[string]interface{}
	for _, src := range tubeSources {
		if !containsSource(sources, src.Key()) {
			continue
		}
		tubeCatalogs = append(tubeCatalogs, externalManifestCatalogs(tubeCatalogDefs(src, tubeOpts[src.Key()]), disabled, homeGenre)...)
	}
	...
	nonStudio := make([]map[string]interface{}, 0,
		len(pornripsCatalogs)+len(hentaiCatalogs)+len(sukebeiCatalogs)+len(stripchatCatalogs)+len(tubeCatalogs)+len(categoryCatalogs)+len(tpbMainCatalogs))
	...
	nonStudio = append(nonStudio, tubeCatalogs...)
	...
	// Broadest search-only catalogs must lead the array: stremio-web's search
	// page lazy-loads rows by visible DOM range and removes EmptyContent rows
	// from the DOM, so when the first catalogs resolve empty the recomputed
	// range maps back onto already-loaded indexes and the remaining rows are
	// never requested (rows stuck as placeholders). tpdb_search and the global
	// piratebay search return results for most queries; both are search-required
	// so hoisting them does not affect Board/Discover ordering.
	searchRank := func(c map[string]interface{}) int {
		switch c["id"] {
		case "tpdb_search":
			return 0
		case PornSearchCatalogID:
			return 1
		default:
			return 2
		}
	...
	sort.SliceStable(catalogs, func(i, j int) bool {
		return searchRank(catalogs[i]) < searchRank(catalogs[j])
	})
	...
	manifest := map[string]interface{}{
		...
		// the configured source). hse- was dropped (HentaiSea excluded). The
		// tube-source prefixes (pvz: / fpv: / ...) are appended from the
		// registered sources in registration order.
		"idPrefixes": append([]string{"jstrm:", "jstrg:", "porndb:", "stash:", "hs:", "hmm-", "sc:"}, tubeIDPrefixes(tubeSources)...),
		"catalogs":   catalogs,
