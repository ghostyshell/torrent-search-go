...
func TestBuildManifestSukebeiWithUserStashKey(t *testing.T) {
	...
	manifest := BuildManifest(cfg, "", nil, nil, nil)
...
func TestBuildManifestFiltersEnabledSorts(t *testing.T) {
	...
	manifest := BuildManifest(cfg, "", &appconfig.Config{}, nil, nil)
...
func TestBuildManifestCatalogOrderSorted(t *testing.T) {
	// Board order: non-TPB-Studio catalogs first (alphabetical), then TPB-Studio
	// catalogs (alphabetical). The TPB-Studio block = piratebay studio catalogs
	// (IDs prefixed "xxx_studio_") plus the dedicated "search" catalog. The main
	// XXX/Trans browse catalogs are TPB-sourced but sort in the first block.
	cfg := Config{
	...
	isStudio := func(c map[string]interface{}) bool {
		id, _ := c["id"].(string)
		return id == PornSearchCatalogID || strings.HasPrefix(id, "xxx_studio_")
	}
...
func TestBuildManifestSearchOnlyOnPart1(t *testing.T) {
	...
	unsplit := BuildManifest(Config{Sources: []string{"piratebay"}, EnabledSorts: []string{"top"}}, "", &appconfig.Config{}, nil, nil)
	...
	part1 := BuildManifest(Config{Sources: []string{"piratebay"}, EnabledSorts: []string{"top"}, Group: 1, GroupTotal: 3}, "", &appconfig.Config{}, nil, nil)
	if !hasSearch(part1["catalogs"].([]map[string]interface{})) {
		t.Fatal("expected Search catalog on part 1")
	}

	// Part 2 (group 2): Search absent.
	part2 := BuildManifest(Config{Sources: []string{"piratebay"}, EnabledSorts: []string{"top"}, Group: 2, GroupTotal: 3}, "", &appconfig.Config{}, nil, nil)
...
func TestBuildManifestTpdbSuppressedByEmptyCategories(t *testing.T) {
	...
	empty := BuildManifest(Config{Sources: []string{"piratebay"}, EnabledSorts: []string{"top"}, TpdbCategories: []string{}}, "", env, nil, nil)
	...
	filled := BuildManifest(Config{Sources: []string{"piratebay"}, EnabledSorts: []string{"top"}, TpdbCategories: []string{"parody"}}, "", env, nil, nil)
