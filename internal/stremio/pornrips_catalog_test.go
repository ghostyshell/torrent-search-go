...
// entries, so we can assert fetchPornripsFromStore dispatches to the right method
// with a normalized studio/tag token.
...
func (f *fakePornripsStore) GetPornripsRecent(ctx context.Context, skip, limit int) ([]prmodels.PornripsEntry, error) {
	f.recentCalled = true
	f.lastSkip, f.lastLimit = skip, limit
	return f.recent, nil
}
func (f *fakePornripsStore) GetPornripsByStudio(ctx context.Context, studioNorm string, skip, limit int) ([]prmodels.PornripsEntry, error) {
	f.lastStudioNorm = studioNorm
	f.lastSkip, f.lastLimit = skip, limit
	return f.studio, nil
}
func (f *fakePornripsStore) GetPornripsByTag(ctx context.Context, tags []string, skip, limit int) ([]prmodels.PornripsEntry, error) {
	f.lastTagNorm = tags
	f.lastSkip, f.lastLimit = skip, limit
	return f.tag, nil
}
func (f *fakePornripsStore) SearchPornrips(ctx context.Context, query string, skip, limit int) ([]prmodels.PornripsEntry, error) {
...
func TestEntriesToCatalogPosterFallback(t *testing.T) {
	out := entriesToCatalog([]prmodels.PornripsEntry{
...
func TestEntriesToCatalogCarriesTorrent(t *testing.T) {
	...
	got := entriesToCatalog([]prmodels.PornripsEntry{e})
	if len(got) != 1 {
