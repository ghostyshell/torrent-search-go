...
// the raw title search rather than stalling the whole request. Sub-deadline of
// searchScrapeTimeout (the global `search` cold cap); 3s leaves ~6s of the 9s
// budget for the piratebay fanout, so a slow StashDB no longer eats the whole
// scrape and leaves the user with zero results.
const searchResolveTimeout = 3 * time.Second
