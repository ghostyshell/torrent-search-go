...
var (
	// qualityPatterns is an ordered list, NOT a map: detection must be
	// deterministic and priority-ordered (2160p -> 1080p -> 720p -> 480p) so a
	// broader lower-res rule never fires before a narrower higher-res one. The
	// 720p `\bhd\b` rule matches the "HD" inside "FULL HD"; if it ran first it
	// would tag 1080p content as 720p and drop it out of every quality-scoped
	// xxx_ catalog. "full hd" (space), "full.hd", and "fullhd" all tag 1080p;
	// the prior literal-dot `full\.?hd` missed the space form.
	qualityPatterns = []struct {
		qual string
		re   *regexp.Regexp
	}{
		{"2160p", regexp.MustCompile(`(?i)\b(2160p|4k|uhd|ultra\.?hd)\b`)},
		{"1080p", regexp.MustCompile(`(?i)\b(1080p|fhd|full[\.\s_-]?hd)\b`)},
