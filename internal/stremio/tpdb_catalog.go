...
// cfg.Sources; the extra indexers (knaben_adult/bitsearch/xxxclub/mypornclub)
...
func matchedSourcesFilter(cfg Config) []string {
	...
	if cfg.ExtraIndexers {
		out = append(out, "knaben_adult", "bitsearch", "xxxclub", "mypornclub")
	}
	if cfg.Enable1337x {
...
// user's configured sources resolved. Redis-fronted catalog cache (same key as
// the prior live path so a warm cache survives the cutover). Empty on a cold
// store (EnrichedScenes nil) - no live fallback, by design (the bulk fill runs
// before the catalog reads flip).
func (h *Handler) serveTPDBBrowse(ctx context.Context, cfg Config, skip int) (CatalogResponse, error) {
	store := newRedisStore(h.Redis)
	cacheKey := prefixTPDBCatalog + "tpdb_new||" + itoa(skip)
	if store != nil {
...
// the pornrips_entries store (pr_search pipeline) when TPDB has no scene. The
// source-resolvable filter the old live path ran is gone - the matched_sources
// gate now lives on the store-backed tpdb_new browse; a live search returns every
// TPDB hit unfiltered, then the on-demand upsert records which sources resolved.
// The pornrips fallback only runs when the user's cfg.Sources actually contains
// pornrips (the gate-bug fix: the old path fell back to pornrips regardless of
// cfg.Sources).
func (h *Handler) serveTPDBSearch(ctx context.Context, cfg Config, contentType, searchQ string, skip int) (CatalogResponse, error) {
	store := newRedisStore(h.Redis)
	cacheKey := prefixTPDBCatalog + "tpdb_search|" + searchQ + "|" + itoa(skip)
	if store != nil {
		if cached, _ := store.getProxiedMetas(ctx, cacheKey); len(cached) > 0 {
			return CatalogResponse{Metas: cached}, nil
		}
	...
	if tpdb := h.tpdbClient(); tpdb != nil {
		...
		if err == nil && len(items) > 0 {
			h.enqueueEnrichedScenesOnDemand(cfg, items)
			resp, _ := h.tpdbMetasFromItems(ctx, store, cacheKey, items, nil)
			return resp, nil
		}
	}
...
// upserts the live TPDB search items into enriched_scenes and torrent-matches
// them against the user's configured sources, so the next browse of tpdb_new is
// store-backed for these scenes. No-op when no callback is wired, no sources are
// configured, or the semaphore is busy. The search response is already rendered;
// this only affects subsequent opens.
func (h *Handler) enqueueEnrichedScenesOnDemand(cfg Config, items []map[string]interface{}) {
	...
	go func() {
		...
		defer cancel()
		h.EnrichedScenesOnDemand(ctx, items, sources)
	}()
}

// tpdbMetasFromItems builds + caches the TPDB-scene metas for live items, used by
// the tpdb_search TPDB-hit branch. Returns empty on err or no items. No
// source-resolvable filter - live search results are unfiltered; the gate is the
// store-backed browse (serveTPDBBrowse).
