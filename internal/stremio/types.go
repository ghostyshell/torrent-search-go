...
type catalogTorrent struct {
	...
	// Members, when non-nil, marks this row as a scene group carrying every
	// resolution variant of one scene; buildMetas emits a jstrg: group id
	// (EncodeGroupID) instead of a per-record jstrm: id so the stream route
	// returns one stream per variant. nil for single-item rows (jstrm: id).
	// The representative fields above (Title/CoverImage/InfoHash/...) come from
	// the highest-resolution member. Persisted in the Redis catalog cache.
	Members []TorrentRecord `json:"members,omitempty"`
}
