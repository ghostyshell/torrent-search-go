...
func main() {
	...
	scraperService.RegisterScraper("1337x", scraper.NewX1337Cache(scraper.NewX1337Scraper(scraperService.GetHTTPClient())))
	...
	scraperService.RegisterScraper("mypornclub", scraper.NewMypornClubScraper(scraperService.GetHTTPClient()))

	// Initialize Magnetio stream service
	streamHTTP := streams.NewHTTPClient(scraperService.GetHTTPClient())
	...
	monitoringHandler.SetRedisClient(redisClient)
	jobRunner := jobs.NewRunner(storageProvider, cfg, scraperService)
	jobRunner.SetAtishmkvProvider(atishmkvProvider)
	enqueueMeta := func(title, detailURL, website, infoHash string) {
		jobRunner.EnqueueMetaLookups([]jobs.MetaEnqueueItem{{
			Title:     title,
			DetailURL: detailURL,
			Website:   website,
			InfoHash:  infoHash,
	...
	// Favorites enqueue with priority so MetaEnricher drains them ahead of the
	// browse/search backlog (the shared queue otherwise grows faster than it
	// drains and user-facing favorites covers would never surface).
	enqueueMetaPriority := func(title, detailURL, website, infoHash string) {
		jobRunner.EnqueueMetaLookups([]jobs.MetaEnqueueItem{{
			Title:     title,
			DetailURL: detailURL,
			Website:   website,
	...
	torrentHandler.MetaEnqueuer = enqueueMeta
	favoritesHandler.MetaEnqueuer = enqueueMetaPriority
	...
	router := middleware.NewRouter(
		...
		middleware.WithMaxBodySize(2<<20), // 2 MiB global cap; handlers needing more override locally
	)
...
func registerStremioRoutes(router *middleware.Router, scrapers *scraper.Service, runner *jobs.Runner, cfg *config.Config, storage *handlers.StorageProvider, redisClient *redis.Client) {
	...
	h := &stremio.Handler{
		...
		SharedMeta:     storage,
		Hentai:         storage,
		EnrichedScenes: storage,
		Studios:        studios,
		Cover:          cover,
	}
	// Tube-source resolvers come from the runner (nil on cold installs); the
	// adapters serve empty streams then.
	var pvzResolver stremio.PerverzijaStreamResolver
	var fpvResolver stremio.FreepornvideosStreamResolver
	var ypvResolver stremio.YespornStreamResolver
	var wptResolver stremio.WatchpornStreamResolver
	var pecResolver stremio.PorneecStreamResolver
	if runner != nil {
		h.MetaEnqueuer = func(ctx context.Context, items []jobs.MetaEnqueueItem) {
			runner.EnqueueMetaLookups(items)
		}
		h.EnrichedScenesOnDemand = runner.EnrichScenesOnDemand
		h.HentaiResolver = runner.HentaiResolver()
		pvzResolver = runner.PerverzijaScraper()
		fpvResolver = runner.FreepornvideosScraper()
		ypvResolver = runner.YespornScraper()
		wptResolver = runner.WatchpornScraper()
		pecResolver = runner.PorneecScraper()
	}

	// Tube-source registry: one register call per direct-play source. Adding a
	// source = register it here + write its adapter + scraper/store; the
	// manifest/catalog/meta/stream dispatch is registry-driven (no per-source
	// handler stack to copy).
	tubeReg := stremio.NewTubeSourceRegistry()
	tubeReg.Register(stremio.NewPerverzijaSource(storage, pvzResolver))
	tubeReg.Register(stremio.NewFreepornvideosSource(storage, fpvResolver))
	tubeReg.Register(stremio.NewYespornSource(storage, ypvResolver))
	tubeReg.Register(stremio.NewWatchpornSource(storage, wptResolver))
	tubeReg.Register(stremio.NewPorneecSource(storage, pecResolver))
	h.TubeSources = tubeReg
	// Package-level default so the registry-unaware free functions
	// (normalizeSources, isCatalogAllowed) recognize tube sources without edits.
	stremio.SetDefaultTubeSourceRegistry(tubeReg)
...
func registerStaticRoutes(router *middleware.Router) {
	// Serve the Next.js static export (dashboard/out built into static/dashboard).
	// Go 1.22 ServeMux treats "GET /" as the subtree catch-all, so this serves
	// index.html at "/" and hashed "_next/static/*" assets, while every
	// registered "/api/*" pattern wins precedence. Hashed assets are immutable.
...
func registerMonitoringRoutes(router *middleware.Router, handler *handlers.MonitoringHandler, ipAllowlistMiddleware *middleware.IPAllowlistMiddleware, dashboardAuthMiddleware *middleware.DashboardAuthMiddleware, _ *middleware.DDoSGuard) {
	...
	router.Post("/api/monitoring/pornrips-sync-trigger", monitoring(handler.TriggerPornripsSync))
	router.Post("/api/monitoring/hentai-sync-trigger", monitoring(handler.TriggerHentaiSync))
	router.Post("/api/monitoring/enriched-scenes-sync-trigger", monitoring(handler.TriggerEnrichedScenesSync))
	router.Post("/api/monitoring/perverzija-sync-trigger", monitoring(handler.TriggerPerverzijaSync))
	router.Post("/api/monitoring/freepornvideos-sync-trigger", monitoring(handler.TriggerFreepornvideosSync))
	router.Post("/api/monitoring/yesporn-sync-trigger", monitoring(handler.TriggerYespornSync))
	router.Post("/api/monitoring/porneec-sync-trigger", monitoring(handler.TriggerPorneecSync))
	...
	// Redis cache viewer/buster for the admin dashboard.
	router.Get("/api/monitoring/redis-caches", monitoring(handler.GetRedisCaches))
	router.Post("/api/monitoring/redis-caches/bust", monitoring(handler.BustRedisCache))
	router.Post("/api/monitoring/redis-caches/bust-all", monitoring(handler.BustAllRedisCaches))
	...
	// Addon status, features, issues, changelog. Public reads (no auth wrapper) so the
	// adult-addons site can consume them cross-origin; writes are dashboard-gated.
	router.Get("/api/addon-status", handler.GetAddonStatusReports)
	router.Get("/api/addon-status/:addonId", handler.GetAddonStatusReport)
	router.Post("/api/monitoring/addon-status", monitoring(handler.CreateAddonStatusReport))
	router.Put("/api/monitoring/addon-status/:addonId", monitoring(handler.ReplaceAddonStatusReport))
	router.Delete("/api/monitoring/addon-status/:addonId", monitoring(handler.DeleteAddonStatusReport))
}
