...
func (c *Client) Migrate() error {
	...
	indexes := []struct {
		...
		// perverzija_entries + freepornvideos_entries: durable flat stores
		// (one doc per scene, no quality grouping). studios_norm (pvz, multikey)
		// / studio_norm (fpv, single) backs _studio; date -1 backs _recent + all
		// listing sorts; tags_norm / categories_norm multikey backs _tag;
		// performers_norm multikey backs _performer; detail_scraped+date backs
		// the enrich sweep (newest-first).
		{"perverzija_entries", bson.D{{Key: "studios_norm", Value: 1}}, nil},
		{"perverzija_entries", bson.D{{Key: "date", Value: -1}}, nil},
		{"perverzija_entries", bson.D{{Key: "tags_norm", Value: 1}}, nil},
		{"perverzija_entries", bson.D{{Key: "performers_norm", Value: 1}}, nil},
		{"perverzija_entries", bson.D{{Key: "detail_scraped", Value: 1}, {Key: "date", Value: -1}}, options.Index().SetName("pvz_missing_detail")},
		{"freepornvideos_entries", bson.D{{Key: "studio_norm", Value: 1}}, nil},
		{"freepornvideos_entries", bson.D{{Key: "date", Value: -1}}, nil},
		{"freepornvideos_entries", bson.D{{Key: "categories_norm", Value: 1}}, nil},
		{"freepornvideos_entries", bson.D{{Key: "performers_norm", Value: 1}}, nil},
		{"freepornvideos_entries", bson.D{{Key: "detail_scraped", Value: 1}, {Key: "date", Value: -1}}, options.Index().SetName("fpv_missing_detail")},
		// yesporn_entries + watchporn_entries: durable flat stores mirroring
		// perverzija_entries (multi-key studios_norm). studios_norm multikey backs
		// _studio; date -1 backs _recent + all listing sorts; tags_norm / performers_norm
		// multikey back _tag / _performer; detail_scraped+date backs the enrich sweep
		// (newest-first). watchporn is Mac-cron-populated but the deployed backend reads
		// it, so the indexes belong in the shared migration.
		{"yesporn_entries", bson.D{{Key: "studios_norm", Value: 1}}, nil},
		{"yesporn_entries", bson.D{{Key: "date", Value: -1}}, nil},
		{"yesporn_entries", bson.D{{Key: "tags_norm", Value: 1}}, nil},
		{"yesporn_entries", bson.D{{Key: "performers_norm", Value: 1}}, nil},
		{"yesporn_entries", bson.D{{Key: "detail_scraped", Value: 1}, {Key: "date", Value: -1}}, options.Index().SetName("ypv_missing_detail")},
		{"watchporn_entries", bson.D{{Key: "studios_norm", Value: 1}}, nil},
		{"watchporn_entries", bson.D{{Key: "date", Value: -1}}, nil},
		{"watchporn_entries", bson.D{{Key: "tags_norm", Value: 1}}, nil},
		{"watchporn_entries", bson.D{{Key: "performers_norm", Value: 1}}, nil},
		{"watchporn_entries", bson.D{{Key: "detail_scraped", Value: 1}, {Key: "date", Value: -1}}, options.Index().SetName("wpt_missing_detail")},
		// porneec_entries: durable flat store mirroring perverzija_entries
		// (multi-key studios_norm). Populated by the deployed PorneecSync job
		// (porneec.com is reachable from prod egress; no Mac cron). studios_norm
		// multikey backs _studio; date -1 backs _recent + all listing sorts;
		// tags_norm / performers_norm multikey back _tag / _performer (tags stay
		// empty - obfuscated slugs); detail_scraped+date backs the enrich sweep.
		{"porneec_entries", bson.D{{Key: "studios_norm", Value: 1}}, nil},
		{"porneec_entries", bson.D{{Key: "date", Value: -1}}, nil},
		{"porneec_entries", bson.D{{Key: "tags_norm", Value: 1}}, nil},
		{"porneec_entries", bson.D{{Key: "performers_norm", Value: 1}}, nil},
		{"porneec_entries", bson.D{{Key: "detail_scraped", Value: 1}, {Key: "date", Value: -1}}, options.Index().SetName("pec_missing_detail")},
