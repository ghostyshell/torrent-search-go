...
func (c *Client) SetCoverImage(ctx context.Context, torrentKey, imageURL string) error {
	...
	_, err := c.coll("images").ReplaceOne(ctx, bson.M{"_id": doc["_id"]}, doc, options.Replace().SetUpsert(true))
	return err
}
...
// SetCoverImageMeta stores a cover image together with its TPDB/StashDB
// enrichment (cover source, description, and the shared_meta link). storageKey
// is written only when non-empty (S3-backed covers); originalURL falls back to
// the image URL. This is the entry point used by the DescriptionImageCache job
// so torrent-search-ui sees an upgraded cover and description.
func (c *Client) SetCoverImageMeta(ctx context.Context, torrentKey, imageURL, originalURL, storageKey, source, description, metaID string) error {
	...
	if storageKey != "" {
		doc["storage_key"] = storageKey
	}
	if source != "" {
		doc["cover_source"] = source
	}
	if description != "" {
